...
public class SelectTag extends TagSupport
{
	...
	public static SingleObject wrapperLabelValue(Object item, String textProperty, String valueProperty)
	{
		...
		Object renderLabel = (((textProperty != null) && (textProperty.length() > 0))
				? wrapper.getPropertyValue(textProperty) : item);
	...
	public static SingleObject wrapperMapLabelValue(Map.Entry<?, ?> entry, String textProperty, String valueProperty)
	{
		...
		Object renderValue = (((valueProperty != null) && (valueProperty.length() > 0))
				? mapKeyWrapper.getPropertyValue(valueProperty) : mapKey.toString());
		Object renderLabel = (((textProperty != null) && (textProperty.length() > 0))
				? mapValueWrapper.getPropertyValue(textProperty) : mapValue.toString());
		SingleObject so = new SingleObject();
		so.label = renderLabel.toString();
		so.value = renderValue.toString();
		return so;
	...
	public int doStartTag() throws JspException
	{
		...
		try
		{
			...
			{
				pageContext.getOut().print(renderInput4Select(name, items, type, textProperty, valueProperty,
						selected, onchange, multiple, onblur));
			}
			else
			{
				pageContext.getOut().print(renderInput4Multiple(items, inputType, type, name, valueProperty,
						textProperty, selected));
			}
		}
		catch (Exception e)
		{
			logger.error("do EnumTag failed, page:" + ((Servlet) this.pageContext.getPage()).toString()
	...
	protected StringBuffer renderInput4Multiple(Object requestItems, String inputType, String type, String name,
			String valueProperty, String textProperty, String selected)
			throws JspException, IllegalArgumentException, IllegalAccessException, ClassNotFoundException
	{
	...
	protected StringBuffer renderInput4Select(String name, Object requestItems, String type, String textProperty,
			String valueProperty, String selected, Object onchange, boolean multiple, Object onblur)
			throws JspException, IllegalArgumentException, IllegalAccessException, ClassNotFoundException
	{
		String onblurchange = "";
		if (onblur != null)
		...
		sb.append("<select style=\"" + (cssStyle == null ? "" : cssStyle)+"\" class=\"" + (cssClass == null ? "" : cssClass) + "\" id=\"" + name + "\" name=\"" + name
				+ "\" " + onblurchange + " " + _multiple + ">");
	...
	private String renderInputMultuple(String name, String type, String selectValue, SingleObject so)
			throws JspException
	{
		String onblurchange = "";
		if (this.onblur != null)
		...
		if(type.equals("text"))
		{
			if (isChecked(so, selectValue))
			{
				...
				string.append("<input id=\"" + showName + "\" type=\"" + type + "\" name=\"" + showName + "\" value=\"" + so.label
						+ "\" " + onblurchange + "/>");
				string.append("<input id=\"" + name + "\" type=\"hidden\" name=\"" + name + "\" value=\"" + so.value
						+ "\" " + onblurchange + "/>");
			}
		}else if(type.equals("checkbox")||type.equals("radio"))
		{
			if (isChecked(so, selectValue))
			{
				string.append("<label><input id=\"" + name + "\" type=\"" + type + "\" name=\"" + name + "\" value=\"" + so.value
						+ "\" checked=\"checked\" " + onblurchange + "/>" + so.label+"</label>");
			}
			else
			{
				string.append("<label><input id=\"" + name + "\" type=\"" + type + "\" name=\"" + name + "\" value=\"" + so.value
						+ "\" " + onblurchange + "/>" + so.label+"</label>");
			}
